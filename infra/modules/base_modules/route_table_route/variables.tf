...
variable "next_hop_type" {
  ...
  type        = string
  validation {
    condition     = contains(local.valid_next_hop_types, var.next_hop_type)
    error_message = "next_hop_type must be one of: ${join(", ", local.valid_next_hop_types)}."
  }
}
...
locals {
  valid_next_hop_types = ["VirtualNetworkGateway", "VnetLocal", "Internet", "VirtualAppliance", "None"]
}